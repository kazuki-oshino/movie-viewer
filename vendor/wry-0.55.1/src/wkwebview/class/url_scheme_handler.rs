...
// Shiori temporary instrumentation: no changes to request/response contents or flow.
fn diagnostic(task_key: usize, stage: &str, details: std::fmt::Arguments<'_>) {
  use std::io::Write;
  let millis = std::time::SystemTime::now()
    .duration_since(std::time::UNIX_EPOCH)
    .unwrap_or_default()
    .as_millis();
  let _ = writeln!(std::io::stderr(), "[asset unixMs={millis} task={task_key:x} {stage}] {details}");
}
...
extern "C" fn start_task(
  ...
  unsafe {
    ...
    if let Some(function) = function {
      ...
      let diagnostic_asset = uri.starts_with("asset:");
      let diagnostic_started = std::time::Instant::now();
      if diagnostic_asset {
        diagnostic(task_key, "REQUEST_NS", format_args!("uri={uri:?} method={:?} headers={:?}", request.HTTPMethod(), request.allHTTPHeaderFields()));
      }
      ...
      match http_request.body(sent_form_body) {
        Ok(final_request) => {
          if diagnostic_asset {
            diagnostic(task_key, "REQUEST", format_args!("method={} uri={} headers={:?}", final_request.method(), final_request.uri(), final_request.headers()));
          }
          ...
          let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
            Box::new(move |sent_response| {
              if diagnostic_asset {
                diagnostic(task_key, "RESPONSE", format_args!("elapsedUs={} status={} bodyBytes={} headers={:?}", diagnostic_started.elapsed().as_micros(), sent_response.status(), sent_response.body().len(), sent_response.headers()));
                if sent_response.status().is_client_error() || sent_response.status().is_server_error() {
                  diagnostic(task_key, "ERROR_RESPONSE_BODY", format_args!("{}", String::from_utf8_lossy(&sent_response.body()[..sent_response.body().len().min(8192)])));
                }
              }
              ...
              if let Err(_e) = validate() {
                if diagnostic_asset {
                  diagnostic(task_key, "CANCELLED_BEFORE_RESPONSE", format_args!("{_e:?}"));
                }
                #[cfg(feature = "tracing")]
              ...
              unsafe fn response(
                ...
                let diagnostic_asset = url.absoluteString().is_some_and(|uri| uri.to_string().starts_with("asset:"));
                // Validate
                check_webview_id_valid(webview_id)?;
                check_task_is_valid(&webview, task_key, task_uuid.clone())?;
                ...
                if diagnostic_asset {
                  diagnostic(task_key, "WK_RESPONSE", format_args!("status={} headers={:?}", response.statusCode(), response.allHeaderFields()));
                }
                ...
                .map_err(|_e| {
                  if diagnostic_asset { diagnostic(task_key, "EXCEPTION_didReceiveResponse", format_args!("{_e:?}")); }
                  crate::Error::CustomProtocolTaskInvalid
                })?;
                if diagnostic_asset { diagnostic(task_key, "didReceiveResponse_OK", format_args!("")); }
                ...
                .map_err(|_e| {
                  if diagnostic_asset { diagnostic(task_key, "EXCEPTION_didReceiveData", format_args!("{_e:?}")); }
                  crate::Error::CustomProtocolTaskInvalid
                })?;
                if diagnostic_asset { diagnostic(task_key, "didReceiveData_OK", format_args!("bodyBytes={}", content.len())); }
                ...
                .map_err(|_e| {
                  if diagnostic_asset { diagnostic(task_key, "EXCEPTION_didFinish", format_args!("{_e:?}")); }
                  crate::Error::CustomProtocolTaskInvalid
                })?;
                if diagnostic_asset { diagnostic(task_key, "didFinish_OK", format_args!("")); }
              ...
              if let Err(_e) = response(
                ...
                if diagnostic_asset { diagnostic(task_key, "NATIVE_RESPONSE_ERROR", format_args!("{_e:?}")); }
                #[cfg(feature = "tracing")]
        ...
        Err(error) => {
          if diagnostic_asset { diagnostic(task_key, "REQUEST_BUILD_ERROR", format_args!("{error:?}")); }
          respond_with_404()
        },
      };
    } else {
...
extern "C" fn stop_task(
  ...
  unsafe {
    if let Some(uri) = task.request().URL().and_then(|url| url.absoluteString()) {
      if uri.to_string().starts_with("asset:") {
        diagnostic(task.hash(), "STOP", format_args!("uri={uri}"));
      }
    }
  }
