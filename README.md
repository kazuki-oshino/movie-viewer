...
- 動画ごとの再生位置・再生速度・色調を復元。
- 明るさ・コントラスト・彩度の調整と、しおりごとの色調保存・復元。
- 1〜4倍の拡大表示と、ドラッグによる表示位置の移動。
- タイムラインにカーソルを合わせて、その時刻の映像をプレビュー。
...
- しおり散歩。検索・色・動画で絞り込んだしおりを、重複しないランダムな順でめくれます。
...
区間しおりは、棚・動画横の一覧・しおり散歩のどこから開いても、Aからリピート再生します。
設定欄を閉じてもリピートは続きます。別の動画を開くと一時的な区間設定はリセットしますが、保存した区間しおりは再起動後も残ります。
折り返しは動画の再生イベントに従うため、フレーム単位の精度や音の継ぎ目の連続性は保証しません。

棚の「しおり散歩」は、その時点の検索・色・動画の絞り込みを使います。
「次のしおり」「戻る」で一枚ずつ見返し、「この瞬間から再生」で該当時刻へ移動できます。
一巡後は「もう一巡する」で順番を変えて続けられます。
場所が不明・内容が変更済み・アクセス不可と判定されている動画は候補から除外します。
閲覧順は一時的なもので、保存済みのしおりや棚の並び順を変更しません。

### 色調・拡大・タイムラインのプレビュー

再生画面右上の「映像の調整」で、明るさ・コントラスト・彩度を変更できます。
「元の色で比較」で元の映像と見比べ、「色調をリセット」で標準に戻せます。
色調は動画ごとに記憶し、地点・A–B区間のどちらのしおりにも作成時の表示色を保存します。
しおりを開くと保存した色調を復元し、一覧のサムネイルにも同じ色調を表示します。
編集画面の「このしおりに保存する色調」から変更できます。以前の色調を持たないしおりは元の色で開きます。
元動画とサムネイル画像そのものは加工しません。

同じパネルで1〜4倍に拡大し、映像をドラッグするか方向ボタンで見たい場所へ移動できます。
「1×」で全体表示へ戻ります。拡大率と表示位置は現在の再生画面だけに適用します。

タイムラインにカーソルを合わせると、その時刻のフレームと時刻を表示します。
キーボードでシークバーにフォーカスしても表示でき、矢印キーで位置を変更できます。
プレビューは別の無音の動画要素で取得するため、現在の再生位置や区間リピートを動かしません。
取得できない場合は案内を表示します。
...
v4導入時はユーザー指定により自動テスト・lintを省略しました。今回、v4の保存パスを含む診断契約にテストを合わせて修正し、テストとlintを再実行しています。
